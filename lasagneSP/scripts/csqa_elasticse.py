...
import sys
sys.path.append("/home/s1959796/debug_lasange/LASAGNE-master")
...
es = Elasticsearch([{'host': 'localhost','port': 9200}], timeout=300, max_retries=100, retry_on_timeout=True)
...
for i, (id, label) in enumerate(kg_entities):
    ...
    if (i+1) % 10000 == 0: 
        print(f'==> Finished {((i+1)/len(kg_entities))*100:.4f}% -- {time.perf_counter() - tic:0.2f}s')
        print(i, flush=True)
        time.sleep(3)
