...
class Inference(object):
    ...
    def construct_actions(self, inference_data, predictor, logger):
        ...
        for i, sample in enumerate(question_type_inference_data):
            ...
            actions = ' '.join([action[1] for action in actions])
            gold_actions = ' '.join([action[1] for action in sample[GOLD_ACTIONS]])
...
def save_checkpoint(state):
    ...
    filename = f'{args.snapshots}/{MODEL_NAME}_e{state[EPOCH]}_v{state[CURR_VAL]:.4f}_{args.task}.pth.tar'
